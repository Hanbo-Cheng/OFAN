...
from decoder_optical import Gru_cond_layer, Gru_prob
...
class Encoder_Decoder(nn.Module):
    def __init__(self, params):
        ...
        self.encoder_off = DenseNet(growthRate=params['growthRate'], reduction=params['reduction'],
                                bottleneck=params['bottleneck'], use_dropout=params['use_dropout'])
        self.encoder_on = DenseNet(growthRate=params['growthRate'], reduction=params['reduction'],
                                bottleneck=params['bottleneck'], use_dropout=params['use_dropout'], input_channel=3)
        self.init_GRU_model = FcLayer(params['D'] * 2, params['n'])
    ...
    def forward(self, params, x, x_mask, y, y_mask, one_step=False):
        ...
        ctx_offline, ctx_mask = self.encoder_off(offline_x, x_mask)
        ctx_online , ctx_mask = self.encoder_on(online_x, x_mask)

        ctx_init = torch.concat((ctx_offline, ctx_online), dim = 1)

        # init state
        ctx_mean = (ctx_init * ctx_mask[:, None, :, :]).sum(3).sum(2) / ctx_mask.sum(2).sum(1)[:, None]
        init_state = self.init_GRU_model(ctx_mean)

        # two GRU layers
        emb = self.emb_model(params, y)
        h2ts, cts, alphas_off, alphas_on, _alpha_pasts_off, _alpha_pasts_on = self.gru_model(params, emb, y_mask, ctx_offline, ctx_online, ctx_mask, one_step, init_state)
        ...
        return scores, [alphas_off, alphas_on]

    # decoding: encoder part
    def f_init(self, x, x_mask=None):
        if x_mask is None:
            shape = x.shape
            x_mask = torch.ones((shape[0],shape[-2],shape[-1])).cuda()
        x_off = x[:,0,:,:][:,None,:,:]
        x_on = x[:,1:-1,:,:]
        ctx_offline, ctx_mask = self.encoder_off(x_off, x_mask)
        ctx_online , ctx_mask = self.encoder_on(x_on, x_mask)
        ctx_init = torch.concat((ctx_offline, ctx_online), dim = 1)
        # init state
        ctx_mean = (ctx_init * ctx_mask[:, None, :, :]).sum(3).sum(2) / ctx_mask.sum(2).sum(1)[:, None]
        init_state = self.init_GRU_model(ctx_mean)
        return init_state, ctx_offline, ctx_online

    # decoding: decoder part
    def f_next(self, params, y, y_mask, ctx_offline, ctx_online, ctx_mask, init_state, alpha_past_off, alpha_past_on, one_step):
        emb_beam = self.emb_model(params, y)

        # one step of two gru layers
        next_state, cts, alphas_off, alphas_on, _alpha_pasts_off, _alpha_pasts_on = self.gru_model(params, emb_beam, y_mask, ctx_offline, ctx_online, ctx_mask, one_step, init_state, alpha_past_off, alpha_past_on)
        ...
        return next_probs, next_state, _alpha_pasts_off, _alpha_pasts_on
