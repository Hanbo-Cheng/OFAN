...
datasets = ['./CROHME/train_images.pkl', r'./CROHME/train_labels.txt']
valid_datasets = ['./CROHME/14_test_images.pkl', './CROHME/14_test_labels.txt']
valid_output = ['./result/valid_decode_result.txt']
valid_result = ['./result/valid.wer']
saveto = r'./result/WAP_params.pkl'

result_path = './wap-14/'
pretrained_model_path = saveto
if not os.path.exists(result_path):
    os.mkdir(result_path)

if not os.path.exists('./result'):  #判断是否存在文件夹如果不存在则创建为文件夹
    os.makedirs('./result')
...
if use_pretrianed_model:
    ...
    WAP_model.load_state_dict(torch.load(pretrained_model_path, map_location=lambda storage, loc: storage))
            ...
            with torch.no_grad():
                ...
                for x, y in valid:
                    for xx in x:
                        ...
                        xx_pad = torch.from_numpy(xx_pad[None, None, :, :]).cuda()  # (1,1,H,W)
